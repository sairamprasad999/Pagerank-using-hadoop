...
public class PageRank {
	...
	for(int i = 0; i < 2*numRepititions; i++) {
		...
		// Set up leftover and size
	    	leftover = job.getCounters().findCounter(Counters.MASSCOUNT).getValue();
	    	System.out.println("########Left over mass ="+leftover+" ################");
	    	size = job.getCounters().findCounter(Counters.NODECOUNT).getValue();
